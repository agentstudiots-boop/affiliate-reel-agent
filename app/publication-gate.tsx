...
export function PublicationGate({job,password}:{job:ContentJob;password:string}){
  ...
  async function request(){setBusy(true);setError("");setNotice("");try{
  ...
  return <section className="reviewBox"><h3>Facebook-Beitrag · separate Freigabe</h3>
    <p>Das System erstellt eine eigene Textgrafik aus dem freigegebenen Inhalt und sendet Beitrag und Bild zur WhatsApp-Freigabe. Nach „Freigeben“ versucht es die Veröffentlichung genau einmal.</p>
    {!record&&<button type="button" disabled={busy||!password} onClick={request}>{busy?"Vorbereitung läuft …":"Beitrag vorbereiten & WhatsApp-Freigabe anfragen"}</button>}
    {record&&<><p>Status: <strong>{record.status}</strong></p>{record.imageUrl&&<a href={record.imageUrl} target="_blank" rel="noopener noreferrer">Grafik prüfen ↗</a>}
      ...
      {record.status==="pending"&&<button type="button" disabled={busy} onClick={request}>{busy?"Bitte warten …":"WhatsApp-Freigabe senden / Status prüfen"}</button>}
