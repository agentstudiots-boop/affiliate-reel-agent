...
import {interpretInstruction,validateInstruction,clarification,type Instruction} from "./instruction";
...
export async function processOperatorInstruction(input:OperatorMessage,
  ...
  const claim=await db.transaction(async sql=>{
    ...
    const targets=await sql.query(`SELECT j.id,j.snapshot,p.id AS publication_id FROM content_jobs j
      ...
      AND ($1::text IS NULL OR p.whatsapp_message_id=$1 OR d.whatsapp_message_id=$1)
      ORDER BY j.updated_at DESC LIMIT 2 FOR UPDATE OF j`,[input.replyToMessageId,trusted]);
    ...
    const row=targets.rows.length===1 && !competing.rows.length?targets.rows[0]:null;
  ...
  if(claim.job){
    try{instruction=validateInstruction(await interpret(input.body,claim.job),input.body);}catch{instruction=clarification();}
    await db.query("UPDATE whatsapp_instructions SET status='parsed',interpretation=$2,updated_at=now() WHERE message_id=$1 AND status='parsing'",[input.id,JSON.stringify(instruction)]);
  }
  let notice=clarifyText,dailyJobId:string|null=null;
  if(claim.job){
    try{
      ...
      const allowed=['visual_context_mismatch','revision_not_available','revision_unchanged','stale_instruction_context','creative_review_failed'];
  ...
  if(claimedNotice.rows.length){
    if(dailyJobId)await sendApproval(dailyJobId);else await send(notice);
  }
  return true;
}
