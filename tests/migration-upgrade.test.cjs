...
test('006–011 upgrade populated 001–005 once without replaying or changing prior migrations', async () => {
  const pg = new PGlite();
  const db = {
  ...
  try {
    ...
    assert.deepEqual(await applyMigrations(db, trackedLoader), {
      applied: ['006_daily_notification.sql', '007_publication_revisions.sql', '008_weekly_reports.sql', '009_original_visual_attempts.sql', '010_replicate_visual_provider.sql', '011_instagram_reel_publications.sql'], alreadyApplied: previous,
    });
    assert.deepEqual(await applyMigrations(db, trackedLoader), {
      applied: [], alreadyApplied: [...previous, '006_daily_notification.sql', '007_publication_revisions.sql', '008_weekly_reports.sql', '009_original_visual_attempts.sql', '010_replicate_visual_provider.sql', '011_instagram_reel_publications.sql'],
    });
    assert.deepEqual(loaded, ['006_daily_notification.sql', '007_publication_revisions.sql', '008_weekly_reports.sql', '009_original_visual_attempts.sql', '010_replicate_visual_provider.sql', '011_instagram_reel_publications.sql'], 'existing migration SQL must never be replayed');
