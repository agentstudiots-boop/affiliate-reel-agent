...
test('Facebook publication needs a distinct signed WhatsApp decision and claims exactly one external POST',async()=>{
  ...
  try{
    ...
    assert.equal(job.content.format,'image');await approveContent(db,memory,id);
    ...
    assert.equal(groupJob.marketing.primary,'Gruppenbeitrag');await approveContent(db,memory,groupId);
    ...
    await runContentJob(instagramOpportunity,{id:instagramId,onUpdate:memory.save,loadLearning:memory.learn});await approveContent(db,memory,instagramId);
    ...
    await runContentJob(opportunity,{id:id2,onUpdate:memory.save,loadLearning:memory.learn});await approveContent(db,memory,id2);
